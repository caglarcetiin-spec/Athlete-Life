...
let texture=null;
let modelReady=false,lastStim={},loadingModel=false,modelError=null,modelAttempts=0;
...
function modelLoaded(){loadingModel=false;modelError=null;modelAttempts=0;["modelFileFallback","bodyMapProtocolWarning","bodyMapRetry"].forEach(id=>{if(q(id))q(id).hidden=true;});status("Hazır · uygulama kütüphanesi");}
function fetchModel(){
 if(loadingModel||!gl)return;loadingModel=true;modelAttempts++;setLoading(true,"Kütüphanedeki beden modeli yükleniyor");
 const xhr=new XMLHttpRequest();xhr.open("GET",MODEL_URL,true);xhr.responseType="arraybuffer";xhr.timeout=60000;
 xhr.onprogress=e=>{if(e.lengthComputable)status(`Kütüphane · %${Math.round(e.loaded/e.total*100)}`)};
 xhr.onload=()=>{if((xhr.status===200||xhr.status===0)&&xhr.response?.byteLength)loadArrayBuffer(xhr.response).then(modelLoaded).catch(fail);else fail(new Error("Model dosyası alınamadı (HTTP "+xhr.status+")"))};
 xhr.onerror=()=>fail(new Error("Uygulama sunucusuna erişilemiyor"));xhr.ontimeout=()=>fail(new Error("Model yükleme zaman aşımı"));try{xhr.send()}catch(e){fail(e)}
}
function fail(err){
 console.error("BodyMap3D",err);loadingModel=false;modelError=err?.message||"Model çözümlenemedi";
 if(location.protocol!=="file:"&&modelAttempts<2){setTimeout(fetchModel,1500);return;}
 setLoading(false);status(modelError,true);const retry=q("bodyMapRetry");if(retry)retry.hidden=false;
 const f=q("modelFileFallback");if(f)f.hidden=false;const w=q("bodyMapProtocolWarning");if(w)w.hidden=location.protocol!=="file:";
}
...
function update(stim){lastStim=stim||{};if(typeof selectedMuscleKey!=="undefined"&&selectedMuscleKey&&KEY_TO_ID[selectedMuscleKey])selectedId=KEY_TO_ID[selectedMuscleKey];const note=q("overlayModeNote");if(note&&typeof overlayModeDescription==="function")note.textContent=overlayModeDescription();resize()}
function setOverlayMode(){requestDraw()}
function init(){
 if(gl){resize();return;}
 try{if(!initGL()){setLoading(false);return;}}catch(e){setLoading(false);status("3D çizim başlatılamadı: "+e.message,true);return;}
 resize();if(typeof ResizeObserver!=="undefined")new ResizeObserver(resize).observe(canvas);
 q("bodyMapRetry")?.addEventListener("click",()=>{modelAttempts=0;fetchModel();});
 if(location.protocol==="file:"){const w=q("bodyMapProtocolWarning");if(w)w.hidden=false;const f=q("modelFileFallback");if(f)f.hidden=false;}fetchModel();
}
window.BodyMap3D={init,update,setOverlayMode,selectMuscle:key=>selectRegion(KEY_TO_ID[key]||0),focusPainEntry,refreshPainHotspots,diagnostics:()=>({regionCount:REGION_KEYS.filter(Boolean).length,jointAnchors:Object.keys(JOINT_POINTS),modelReady:!!modelReady,webglReady:!!gl,loading:loadingModel,error:modelError,source:MODEL_URL})};
