...
try{
 ...
 await page.getByRole('button',{name:'Bu hatırlatmayı kapat',exact:true}).click();await page.reload();await expect(page.getByText('Kendi yolunu birlikte oluşturalım.',{exact:true})).toHaveCount(0);
 ...
 await page.locator('.page-enter').evaluate(async element=>{await Promise.all(element.getAnimations({subtree:true}).map(animation=>animation.finished))});
