...
function openSession(id=null,sportId=null,plan=null){
 ...
 function drawMetrics(values={}){
  ...
  q('session-metrics').innerHTML=base.map(field).join('')+(model?`<details class="science-metrics" ${extra.some(f=>values[f[0]]!=null)?'open':''}><summary>${esc(model.name)} · ${extra.length?'ayrıntılı ölçümler':'kayıt açıklaması'}</summary><p class="hint">${esc(model.focus)}</p><p class="hint">${extra.length?'Doldurduğun ölçümler kaydedilir ve aynı koşullardaki kayıtlarla analiz edilir.':'Bu modelde ayrı bir özet ölçüm formu yok.'} Hareket ve setlerini aşağıdaki bölümden ekleyebilirsin; planın varsa çalışmayı seç.</p><p class="hint">Karşılaştırma bağlamı: ${esc(model.conditions)}</p><div class="sports-search">${extra.map(field).join('')}</div>${model.optionalTests.length?`<details><summary>Kaydettiğim 200/400 m test sonuçları</summary><p class="hint">Yalnız aynı stil ve havuzdaki mevcut test sonuçlarını gir. Hesap kritik tempo tahminidir; test yapma veya güvenli yüzme önerisi değildir.</p><div class="sports-search">${model.optionalTests.map(field).join('')}</div></details>`:''}</details>`:'');
 }
 let linksDrawn=false;
 function drawLinks(){
  ...
  const oldLink=linksDrawn?q('session-link')?.value:initial.linkedSessionId,oldPlan=linksDrawn?q('session-plan')?.value:initial.planBlockId;
  const link=links.some(r=>r.id===oldLink)?oldLink:'',selectedPlan=plans.some(b=>b.id===oldPlan)?oldPlan:'';
  const changed=!!(oldLink&&!link||oldPlan&&!selectedPlan);
  q('session-links').innerHTML=`<section class="session-connections"><h3>Bu seans neye bağlı?</h3><p class="hint">Bağlantı seçmek zorunlu değil. Ayrı kayıt, yeni bir seans olarak sayılır; aynı hareket kaydıyla eşleştirme çift sayımı önler.</p><div ${links.length?'':'hidden'}><label>Aynı seansın hareket kaydı<select id="session-link"><option value="">Yeni ve bağımsız bir seans</option>${links.map(r=>`<option value="${esc(r.id)}" ${r.id===link?'selected':''}>${esc(r.sportName)} · ${fmtDate(r.date)}${r.durationMin!=null?' · '+esc(r.durationMin)+' dk':''}</option>`).join('')}</select></label><p class="hint">${links.length} uygun kayıt. Yalnız gerçekten aynı seansı seç. Eşleştirmede hareket analizinin kaynağı mevcut hareket kaydı olarak kalır; buradaki setler ikinci kez eklenmez.</p></div>${!links.length?'<p class="session-empty"><strong>Hareket kaydıyla eşleştirme yok.</strong> Bu tarihte eşleştirilebilecek ayrı bir hareket kaydı bulunmuyor. Yeni seans olarak kaydedilecek.</p>':''}<div ${plans.length?'':'hidden'}><label>Planlanan çalışma<select id="session-plan"><option value="">Bir plana bağlamadan kaydet</option>${plans.map(b=>`<option value="${esc(b.id)}" data-period="${esc(b.periodId)}" ${b.id===selectedPlan?'selected':''}>${esc(b.periodName)} · ${esc(b.sportName)} · ${b.durationMin} dk${b.steps?.length?' · '+b.steps.length+' hareket/adım':''}</option>`).join('')}</select></label><p class="hint">${plans.length} çalışma bu tarih ve branşla eşleşiyor. Seçtiğinde varsa hareketleri ve gerçek set alanları aşağıda açılır.</p></div>${!plans.length?'<p class="session-empty"><strong>Bu gün ve branş için planlanan çalışma yok.</strong> Kayıt plan dışında tutulacak. Dönemlerim bölümünde ana plana aldığın çalışmalar, kendi gün ve branşında burada görünür. Önceki kuvvet programının kayıtları Hareket ve set kaydı bölümündedir.</p>':''}<p id="session-link-status" class="hint" role="status">${changed?'Tarih veya branş değiştiği için artık eşleşmeyen bağlantı kaldırıldı. Yeni seçenekleri kontrol et.':''}</p></section>`;
  linksDrawn=true;
 }
