...
function weight(x){if(!x||typeof x!=="object")return 0;let n=0;const maps=["daily","week","weekOptimizations","scheduleByDate","trainingLogs","foodLogs","planHistory","sessionFeedback","painLogs","futurePlans","waterLogs","social","water","sessionPrescriptions","gapReconciliation"];maps.forEach(k=>{const v=x[k];if(v&&typeof v==="object")n+=Object.keys(v).length});["bodyMeasurements","capabilityRecords","adHocSessions","guidedWorkoutHistory","runs","generatedWeekPlan","photoProgress","sportSessions","customSports","athleteProfileHistory","sportSessionHistory","multisportPeriods"].forEach(k=>{if(Array.isArray(x[k]))n+=x[k].length});if(x.athleteProfile?.completedAt)n++;return n}
