...
## Explicit real migration authorization and preview preservation

The user explicitly approved real preview/account migration after the earlier review block. Read-only inventory found one matching preview account with 11 canonical records. One invalid pending program (missing movement name) was exported via the app to a private device journal; it will be archived without replay. The cutover restores canonical preview records before importing legacy sources, preserving the newer active plan. New composer validation prevents queuing unnamed movements.

Synthetic preview cutover tests cover interruption after preview commit, resume, preservation of both old/new hydration records, no duplication, corrupt-input rejection and device-journal archival. The first new test failed because its assertion omitted the correctly imported legacy hydration record; the corrected assertion checks both records and every field of the restored preview record. Original failure logs remain in runs/. GitHub source cb9c391: all core/container/mongodb jobs SUCCESS in run 35913432104. Final source CI and live cutover remain required.

## WAN cutover correction

During the authorized live cutover, two accounts completed; the larger account stopped with the sanitized `storage_retry` error. Old sources remained unchanged and the service stayed in maintenance. Per-record remote FK checks, identity probes and writes were reduced: transaction-local parents satisfy FK lookups, remaining references are cached within a flush, and new documents are grouped into ordered `insert_many` calls. Existing updates and large-document chunks remain in the same majority transaction. Duplicate identities/unique keys still abort with IntegrityError.

A synthetic 350-record cutover with 20 ms delays on collection reads/writes passed in 16.13 seconds; assertions bound round trips and require readiness. This is a network-delay regression, not a test with personal records. Full Mongo regression is rerun before resuming the actual migration. No source data is deleted; completed account markers make the operation resumable.
