...
class AccountHandler(SimpleHTTPRequestHandler):
    ...
    def get(self):
        ...
        if not target.is_relative_to(ROOT) or not target.is_file() or not (path in scripts or path in {'account-entry.css', 'account-guidance.css', 'premium-workspace.css', 'styles.css', 'sports-profile.css', 'account-workspace.css', 'account-design.css'} or is_library or is_asset):
            return self.json(404, {'ok': False, 'error': 'Bulunamadı.'})
        return super().do_GET()
