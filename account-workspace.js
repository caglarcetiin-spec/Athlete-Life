...
function init(){
 ...
 const historyBar=document.createElement('nav');historyBar.className='workspace-history-nav';historyBar.setAttribute('aria-label','Gezinme geçmişi');historyBar.innerHTML='<button type="button" class="secondary" id="workspace-back" aria-label="Önceki sayfaya dön" disabled>← Geri</button><button type="button" class="primary" id="workspace-home">Ana sayfa</button><button type="button" class="secondary" id="workspace-forward" aria-label="Sonraki sayfaya git" disabled>İleri →</button>';document.querySelector('main.main').append(historyBar);
